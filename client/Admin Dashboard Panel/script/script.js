...
document.addEventListener("DOMContentLoaded", async function () {
  try {
    const response = await fetch(
      `https://variable-sculptress-6789-e41a.onrender.com/results`,
      {
        headers: {
        ...
        latestResults.map(async (result) => {
          try {
            const languageResponse = await fetch(
              `${apiUrl}/languages/filter/${encodeURIComponent(
                result.languageName
              )}`
            );
...
const fetchData = async () => {
  try {
    ...
    //   }).then((response) => response.json()),
    // ]);
    // console.log(questions, users, quizPlayed);

    const ques = await fetch(`${apiUrl}/questions`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });

    const Quedata = await ques.json();
    // =============================================
    const users = await fetch(`${apiUrl}/users/user`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
    ...
    // =========================================================
    const quizPlayed = await fetch(`${apiUrl}/results`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });

    const quizData = await quizPlayed.json();
    console.log(quizData);
    document.getElementById("total-question").textContent = Quedata.msg.length;
    document.getElementById("total-users").textContent = userData.length;
    document.getElementById("total-quiz-played").textContent = quizData.length;
  } catch (error) {
